...
| F | ✅ Bitti | boss.dart 1681→**394 sat**'a indi. Saf birimler: F1 PostureSystem (combat/sim), F2 BossBrain (combat/ai, seedli Rng boss'ta paylaşılır), +unit test. F6 BossView (presentation, render). Bağlı çekirdek (F3 temas, F4 deathblow, F5 state machine) `part of boss.dart` + `extension on Boss` ile aynı kütüphanede bölündü → davranış-koruyan saf taşıma (alanlar/statikler Boss'ta, yalnız `Boss.` niteleyici eklendi; diff ile birebir kanıtlandı). F7 ~500 hedefi aşıldı. analyze temiz, 141 test yeşil. game.actionSystem invariant'ı korundu (grep testi part dosyalarını da kapsayacak şekilde genişletildi). Kalan: F8 elle duman testi (kullanıcı). |
| G | ✅ Bitti | Ash Gate dikey kesiti uçtan uca: menü→diyalog→3 seçim→1 zar(1d20+stealth)→Knight 1 combat→win/loss→ödül/flag→menü. Saf çekirdek (ScenarioState/Rng/DiceService/Encounter/EncounterRunner, Flame'siz, 16 test). Akış otoritesi EncounterRunner (soyut EncounterHost → D6 çözüldü). Zar yalnız hikayede: başarı→approached_silently→`bossOpeningDelay` (ArenaActionSystem getter, default 0 → sandbox/normal maç muaf); parry/dodge math'ine dokunulmadı. BossDefeated event'i artık yayılıyor. analyze temiz, 157 test yeşil. Save/load → Faz H. Elle duman: kullanıcı. |
| H | ✅ Bitti | ScenarioState kalıcılığı (shared_preferences). Ports&adapters: `domain/save_state.dart` (saf serialize, version:1) + `domain/save_repository.dart` (saf orkestrasyon + `SaveStore` portu, bozuk/sürüm-uyumsuz→güvenli null) + `core/shared_prefs_save_store.dart` (TEK adaptör, anahtar `scenario_save_v1`). GameSession = TEK persist otoritesi (her mutasyon + runner `onStateChanged`→persist); açılışta `attachPersistence` yükler; `resetProgress` temizler. Menü: Devam et/Yeni oyun (onaylı ConfirmResetOverlay) + ilerleme satırı. Ödül bir kez + tamamlandı ATOMİK (reward ekranında çökse bile çift ödül yok); disk yazımları SIRALI kuyruk (race/reset-clobber yok); start'taki flag temizliği de persist edilir. Input ayarları AYRI (input.* anahtarları). analyze temiz, 178 test yeşil (save_state 5 + save_repository 8 + session 3 + runner ödül/atomik/kanca 3). Codex incelemesi (2 Yüksek + 3 Orta) uygulandı. Elle duman: kullanıcı. |
